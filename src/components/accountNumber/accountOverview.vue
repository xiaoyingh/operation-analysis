<template>
  <div>
    <div class="serch_box">
      ...
      <!--   全选和反选   -->
      <el-checkbox v-if="oneShows == false" v-model="checkAll" style="margin-bottom: 24px;"
      ...
      <el-checkbox-group v-model="selectArr" @change="handleCardList">
        <div v-for="(item,index) in cardLists" :key="index" class="dialogBox">
          ...
          <el-card v-for="(item2,index2) in item.data" :key="index2" shadow="hover">
            <div v-if="oneShows === true" :title="item2.mediaName" class="cardTitle">{{ item2.mediaName }}</div>
            <el-checkbox v-else :label="item2.accountId" :title="item2.mediaName">{{ item2.mediaName }}</el-checkbox>
          </el-card>
        </div>
      </el-checkbox-group>
    </div>
...
<script>
import { selectManagementAnalysis, getOrganizeName, getAccountAnalysisyi } from '@/api/comparativeAnalysis'
...
export default {
  data() {
    return {
      ...
      weichartData: [{ value: null, text: '全部' }, { value: '3', text: '客户端' }, { value: '4', text: '微信' }, {
        value: '5',
        text: '微博'
      }, { value: '6', text: '头条' }, { value: '7', text: '抖音' }, { value: '8', text: '快手' }, {
        value: '1',
        text: '网站'
      }, { value: '2', text: '电子报纸' }],
  ...
  methods: {
    ...
    getMyyixuan() {
      ...
      getAccountAnalysisyi(obj).then(res => {
        if (res && res.data) {
          this.selectedNum = res.data
          // 如果初始时没有选择 则选中全部
          if (this.selectedNum.length === 0) {
            this.selectedNum = this.allNum.slice(0)
          }
          this.$emit('parentFun', this.selectedNum)
          // 账号回显
    ...
    selectManagementAnalysis() {
      const obj = {
        ...
        pageNum: 1,
        pageSize: 100,
        channelType: this.formInline.region,
      ...
      this.loadingInstance = this.$loading({
        fullscreen: true,
        target: document.querySelector('.serch_box'),
    ...
    handleCardList(value) {
      ...
      for (let i = 0; i < this.selectedNum.length; i++) {
        const obj = {}
        obj.tenantId = 5
        obj.accountId = this.selectedNum[i].accountId
        obj.moduleType = 1
        obj.mediaName = this.selectedNum[i].mediaName
        obj.channelType = this.selectedNum[i].channelType
        arr.push(obj)
      }
      this.$emit('parentFun', arr)
      this.isToggle = this.selectedNum.length > 9
      this.checkAll = this.selectedNum.length === this.allNum.length
    },
    // 删除账号
    delects(index) {
