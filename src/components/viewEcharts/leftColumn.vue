  ...
  <div class="sidebarWarpBox" :class="this.$store.getters.sliderShow ? 'openBox':'closeBox'">
    <div class="sidebarBox">
          ...
          <div v-if="sidebar.search">
            <p v-for="(item,index) in showList" :key="index" class="contextList">
              <el-checkbox-group v-model="checkedCities" @change="handleCheckedCitiesChange">
                ...
                <span v-if="item.isShow" class="zi">自</span>
              </el-checkbox-group>
            </p>
          </div>
          <!--  列表展示 -->
          <div v-else>
            <p v-for="(item,index) in showList" :key="index" :class="{active : active === item.id}" class="contextList" @click="handleList(item.id)">
      ...
      <div v-if="sidebar.search" class="buttons">
        ...
        <el-button type="primary">分 析</el-button>
      </div>
      <!--  账号总览   -->
      <el-dialog
        title="添加对比"
        ...
        width="1096px"
      >
        <ContrastOverview @parentFun="getChildData" />
        <div class="button">
...
export default {
  ...
  mounted() {
    ...
    for (var item in this.showList) {
      this.checkList.push(this.showList[item].id)
    }
  },
  methods: {
    // 请求添加的接口
    ...
    handleList(id) {
      this.active = id
      console.log(id)
      // this.$emit('childLeftID', id)
    },
    querySearch() {
      this.showList = []
      this.list.filter(val => {
        if (val.title.indexOf(this.input) !== -1) {
